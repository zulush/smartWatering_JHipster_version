...
describe('Plante e2e test', () => {
  ...
  const planteSample = { libelle: 'generating' };
  ...
  describe('new Plante page', () => {
    ...
    it('should create an instance of Plante', () => {
      cy.get(`[data-cy="libelle"]`).type('Saint-Marin quantifying').should('have.value', 'Saint-Marin quantifying');

      cy.setFieldImageAsBytesOfEntity('photo', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="racine"]`).type('deposit Haute-Normandie Chicken').should('have.value', 'deposit Haute-Normandie Chicken');
